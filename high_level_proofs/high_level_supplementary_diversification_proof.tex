...
\hline
\textbf{Result} & \textbf{Statement (brief)} & \textbf{Chapter} \\
\hline
Thm.~\ref{thm:naive-concentration} &
  A single-asset portfolio is never variance-minimising unless it has
  zero covariance with every other asset. & 2 \\
\addlinespace
Thm.~\ref{thm:diversification-bound} &
  Portfolio variance satisfies
  $\Var(R_p) \leq \bar{\sigma}^2 / n + (1 - 1/n)\bar{\rho}\,\bar{\sigma}^2$;
  diversification strictly reduces variance when $\bar{\rho} < 1$. & 2 \\
\addlinespace
Thm.~\ref{thm:correlation-regime} &
  Diversification benefit $\Delta V$ is a decreasing function of the
  inter-asset correlation $\rho$; regime shifts that increase $\rho$
  erode the benefit and trigger rebalancing. & 3 \\
\addlinespace
Thm.~\ref{thm:hard-constraint} &
  Under a maximum-concentration hard constraint $w_i \leq w_{\max}$,
  the feasible set is non-empty, compact, and convex; the constrained
  optimum exists and is unique under a strictly convex objective. & 4 \\
\addlinespace
Thm.~\ref{thm:soft-constraint} &
  The Lagrangian relaxation of a soft correlation-band constraint
  yields a closed-form penalty adjustment to the mean-variance
  efficient frontier. & 4 \\
\addlinespace
Thm.~\ref{thm:rebalance-trigger} &
  A rebalancing trigger fires at time $t$ if and only if at least one
  hard constraint is violated or the soft-constraint violation exceeds
  the configured tolerance; the trigger is decidable in $O(n^2)$ time. & 5 \\
\addlinespace
Thm.~\ref{thm:time-horizon-layer} &
  The aggregate risk of a layered portfolio is bounded by the sum of
  per-layer risk budgets; layers are mutually uncorrelated across
  horizons under the stated independence assumption. & 6 \\
\addlinespace
Thm.~\ref{thm:within-bucket} &
  Within any asset-class bucket, internal diversification across
  factors, geographies, and durations reduces bucket variance below
  the variance of any single constituent. & 7 \\
\addlinespace
Thm.~\ref{thm:cross-asset-cvar} &
  Cross-asset CVaR satisfies a sub-additivity inequality; the benefit
  of cross-asset diversification is strictly positive whenever assets
  are not perfectly co-monotonic. & 8 \\
\hline
\end{tabularx}
\end{center}
...
\begin{enumerate}
  ...
  \begin{enumerate}
    ...
    \item \textbf{(Domain and codomain.)}
          By Assumption~\ref{asm:universe}, $\calU$ is a finite,
          non-empty set of assets.
          By Definition~\ref{def:char-map}, $\kappa$ is a
    ...
    \item \textbf{(Single-valuedness.)}
          ...
          no additional assumption beyond Definition~\ref{def:char-map}.
...
\begin{proof}
We establish the result by explicitly constructing a diversifying
perturbation that reduces variance.

\begin{enumerate}
  \item \textbf{Setting.}
        Fix $k$ and let $\bfw = \bfe_k$.  Then
        $V_p(\bfe_k) = \sigma_k^2$.

  \item \textbf{Perturbation.}
        Let $j \neq k$ and define the perturbed portfolio
        $\bfw_\varepsilon = (1-\varepsilon)\bfe_k + \varepsilon\bfe_j$
        for $\varepsilon \in (0,1)$.  This is feasible in $\Delta^{n-1}$.

  \item \textbf{Variance of the perturbed portfolio.}
        \begin{align}
          V_p(\bfw_\varepsilon)
          ...
          \label{eq:nc-var-eps}
        \end{align}
        where $\sigma_{kj} = \Cov(r_k, r_j)$.

  \item \textbf{First-order condition.}
        Differentiating~\eqref{eq:nc-var-eps} with respect to
        $\varepsilon$ at $\varepsilon = 0$:
        \begin{align}
          ...
          \label{eq:nc-foc}
        \end{align}
        The derivative is negative (diversification reduces variance)
        whenever $\sigma_{kj} < \sigma_k^2$, i.e.\ whenever
        $\rho_{kj} < \sigma_k / \sigma_j$.  In particular, if
        $\rho_{kj} < 1$ and $\sigma_j \leq \sigma_k$, this holds.

  \item \textbf{Conclusion.}
        If~\eqref{eq:nc-foc} is negative for any $j$, then
        $V_p(\bfw_\varepsilon) < V_p(\bfe_k)$ for small $\varepsilon > 0$,
        so $\bfe_k$ is not the minimum-variance portfolio.
        The only exception is when $\sigma_{kj} \geq \sigma_k^2$ for all
        $j \neq k$, i.e.\ $\rho_{kj} \geq \sigma_k/\sigma_j$ for all
        $j$, which requires $\Cov(r_k, r_j) \geq \sigma_k^2$ --- a
        condition that cannot hold generically.
\end{enumerate}
\qed
\end{proof}
...
\begin{enumerate}
  \item By Definition~\ref{def:bucket}, $\calD_k \subseteq \calD$
