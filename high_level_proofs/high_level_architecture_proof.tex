...
\begin{proof}
We verify each step explicitly.
\begin{enumerate}
  \item \textbf{Assumption.}
        $f$ is $\calI_s$-measurable, i.e.,
        $f^{-1}(B) \in \calI_s$ for all $B \in \mathcal{B}(\mathbb{R})$.
  \item \textbf{Filtration monotonicity.}
        By Definition~\ref{def:filtration}(1),
        $s \leq t \Rightarrow \calI_s \subseteq \calI_t$.
  \item \textbf{Conclusion.}
        Therefore $f^{-1}(B) \in \calI_s \subseteq \calI_t$ for all
        $B \in \mathcal{B}(\mathbb{R})$, so $f$ is
        $\calI_t$-measurable.\qed
\end{enumerate}
\end{proof}

\begin{lemma}[Future Dependence Implies Non-Measurability]
\label{lem:future-dep}
Let $g : \Omega \to \mathbb{R}$ be $\calI_{t+\tau}$-measurable for
some $\tau > 0$, and suppose $g$ is \emph{not}
$\calI_t$-measurable.  Then no $\calI_t$-measurable function $h$
satisfies $h = g$ $\mathbb{P}$-a.s.
\end{lemma}

\begin{proof}
We proceed by contradiction.
\begin{enumerate}
  \item \textbf{Suppose} there exists an $\calI_t$-measurable
        function $h$ with $h = g$ $\mathbb{P}$-a.s.
  \item Since $(\Omega, \mathcal{F}, \mathbb{P})$ is complete and
        $\calI_t$ contains all null sets
        (Definition~\ref{def:filtration}(2)), $\calI_t$-measurability
        is preserved under $\mathbb{P}$-a.s.\ equality.
  \item Therefore $g$ is $\calI_t$-measurable, contradicting the
        hypothesis.
  \item \textbf{Conclusion.}
        No such $h$ exists.\qed
\end{enumerate}
\end{proof}
